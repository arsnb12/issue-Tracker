...
def create_issue():
   ...
   #  print ( "body={} title={} severity_id={}".format( body, title, severity_id ) )

    s = run_insert("insert into i_issue (id, title, body, severity_id, state_id ) values (%(id)s, %(title)s, %(body)s, %(severity_id)s, %(state_id)s)",
...
def delete_issue():
   ...
   # print ("oki")
    ...
    if "issue_id" in dict:
        ...
        issue_id = 0000 #Default
        print("Here, issue id = {}".format(issue_id))

    s = run_delete ( "DELETE FROM i_note WHERE issue_id = %(id)s; DELETE FROM i_issue WHERE id = %(id)s ", 
...
def update_issue():
    ...
    if "title" in dict:
        ...
        title = dict["title"][0]
        issue_id = dict["issue_id"][0]
        print("updated:: issue id = {}, body = {}, title={}".format(issue_id, body, title))
        s = run_update ( "update i_issue set body = %(body)s, title = %(title)s where id = %(issue_id)s", 
            { "body":body, "title":title, "issue_id":issue_id } )

    else:
        body = dict["body"][0]    
        issue_id = dict["issue_id"][0]
        print("updated:: issue id = {}, body = {}".format(issue_id, body))
        s = run_update ( "update i_issue set body = %(body)s where id = %(issue_id)s",
            { "body":body, "issue_id":issue_id } )
...
def get_issue_detail():
    ...
    issue_data = run_select_raw ("select * from i_issue where id = %(issue_id)s", 
        {"issue_id":issue_id})
...
def create_note():
    ...
    s = run_insert("insert into i_note (issue_id, title, body ) select id, title, body from i_issue where id=%(issue_id)s",
            { "issue_id":issue_id })
    db_conn.commit()
...
def update_note():
    ...
    if not required_param(dict,["note_id","issue_id"]):
       return

    issue_id = dict["issue_id"][0]
    note_id = dict["note_id"][0]
    print ("oki")
    print ("issue_id = {}, note_id={}".format(issue_id, note_id))
    
    s = run_delete ( "DELETE FROM i_note WHERE id = %(note_id)s AND issue_id = %(issue_id)s",
            { "note_id": note_id, "issue_id": issue_id })
...
def upd_severity():
    ...
    s = run_update ( "update i_issue set severity_id = %(severity_id)s where id = %(issue_id)s cascade",
            { "severity_id":severity_id, "issue_id":issue_id } )
    db_conn.commit()
    return s
...
def upd_state():
    ...
    s = run_update ( "update i_issue set state_id = %(state_id)s where id = %(issue_id)s", 
            { "state_id":state_id, "issue_id":issue_id } )
    db_conn.commit()
    return s
